...
void PlayScene::Initialize() {
	// TODO 5 (1/2): There's a bug in this file, which crashes the game when you win. Try to find it. (ok)
	// TODO 5 (2/2): There's a cheat code in this file. Try to find it. (ok)
	mapState.clear();
	keyStrokes.clear();
	ticks = 0;
	lives = 10;
...
void PlayScene::Update(float deltaTime) {
	...
	for (int i = 0; i < SpeedMult; i++) {
		...
		if (enemyWaveData.empty()) {
			if (EnemyGroup->GetObjects().empty()) {
				...
				delete imgTarget;
				// Win.
				Engine::GameEngine::GetInstance().ChangeScene("win"); //bug is here. turn "win-scene" to "win"
			}
			continue;
		}
	...
	//         The format is "[EnemyId] [TimeDelay] [Repeat]". (ok)
	// TODO 2 (8/8): Enable the creation of the 4th enemy. (ok)
        case 4:
...
void PlayScene::ConstructUI() {
	...
	UIGroup->AddNewControlObject(btn);
	// TODO 2 (3/8): Create a button to support constructing the 4th tower. (ok)
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1522, 136, 0, 0, 0, 0),
		Engine::Sprite("play/turret-4.png", 1522, 136, 0, 0, 0, 0)
		, 1522, 136, MachineGunTurret2::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	// TODO 2 (4/8): On callback, create the 4th tower. (ok)
	else if (id == 3 && money >= MachineGunTurret2::Price)
        preview = new MachineGunTurret2(0, 0);
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		for (auto &dir : PlayScene::directions) {
            int x = p.x + dir.x;
            int y = p.y + dir.y;
            if (x<0 || y<0 || x>=PlayScene::MapWidth || y>=PlayScene::MapHeight) {
                continue;
            }
