...
void Slider::Draw() const {
	// TODO 4 (1/6): Draw all components. (ok)
	Image::Draw();
	Bar.Draw();
	End1.Draw();
	End2.Draw();
...
void Slider::OnMouseMove(int mx, int my) {
	...
	float Length = End2.Position.x - End1.Position.x;
	if (Down) {
        if (mx>=End1.Position.x && mx<=End2.Position.x && my<=Position.y+10 && my>=Position.y-10) {
            Position.x = mx;
            SetValue( (Position.x - End1.Position.x)/Length );
            //cout << (Position.x - End1.Position.x)/Length << endl;
        }
	}
	if (!Down) bmp = imgOut;
	else bmp = imgIn;
